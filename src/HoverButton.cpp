...
void CHoverButton::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if( m_hWndparents != NULL )
	{
		::PostMessage(m_hWndparents,UM_JOG,m_iAxis,-1);	// Stop
	}
...
void CHoverButton::OnLButtonDown(UINT nFlags, CPoint point)
{
	if( m_hWndparents != NULL )
	{
		::PostMessage(m_hWndparents,UM_JOG,m_iAxis,m_iDir);
	}
...
void CHoverButton::SetParentHwnd(HWND hWnd)
{
	m_hWndparents = hWnd;
}
